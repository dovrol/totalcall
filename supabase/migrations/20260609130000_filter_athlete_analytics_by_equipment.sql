-- Filter athlete analytics and the cohort benchmark by equipment (raw vs equipped).
-- athlete_results holds each athlete's full cross-equipment career; a competition is
-- a single discipline (e.g. IPF classic = raw), so equipped results must not pollute
-- its history/analytics. The competition's discipline maps (client-side) to a set of
-- OPL equipment values passed here as p_equipment; null means "no filter" (back-compat).
-- The athlete_history_view stays unfiltered and is filtered per request in PostgREST.

drop function if exists public.get_athlete_analytics(text, text);
...
as $$
  with results as (
    ...
    where a.slug = p_athlete_slug
      ...
      and (p_equipment is null or ar.equipment = any(p_equipment))
  ),
  total_values as (
    select
      r.total_kg,
...
comment on function public.get_athlete_analytics(text, text, text[]) is
  'Public frontend-safe athlete analytics calculated from athlete_results, optionally '
  'restricted to a set of equipment values (e.g. raw-only). Attempt success ignores NULL '
  'and 0 values; positive attempts are successful and negative attempts are missed.';

revoke all on function public.get_athlete_analytics(text, text, text[]) from public;
grant execute on function public.get_athlete_analytics(text, text, text[]) to anon, authenticated;

drop function if exists public.get_attempt_benchmark(text[], text);

create function public.get_attempt_benchmark(
...
comment on function public.get_attempt_benchmark(text[], text, text[]) is
  'Public frontend-safe attempt-success aggregate across a cohort of athlete slugs for a '
  'source, optionally restricted to a set of equipment values (e.g. raw-only). Attempt '
  'success ignores NULL and 0 values; positive attempts are successful and negative attempts are missed.';

revoke all on function public.get_attempt_benchmark(text[], text, text[]) from public;
grant execute on function public.get_attempt_benchmark(text[], text, text[]) to anon, authenticated;
