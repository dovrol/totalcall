-- Public athlete analytics contract for the frontend.
-- Metrics are calculated from imported Supabase result rows and expose only
-- aggregate public competition data.

create or replace function public.get_athlete_analytics(p_athlete_slug text)
returns table (
  athlete_slug text,
  starts_count int,
...
as $$
  ...
  summary as (
    select
      ...
      max(total_kg) filter (where total_kg > 0) as best_total_kg,
...
comment on function public.get_athlete_analytics(text) is
  'Public frontend-safe athlete analytics calculated from athlete_results. Attempt success ignores NULL and 0 values; positive attempts are successful and negative attempts are missed.';

revoke all on function public.get_athlete_analytics(text) from public;
grant execute on function public.get_athlete_analytics(text) to anon, authenticated;
